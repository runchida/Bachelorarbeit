...
def write_mixed_labels(train_log_dir, labels, filename=LABELS_FILENAME):
    labels_filename = os.path.join(train_log_dir, filename)
    with tf.gfile.Open(labels_filename, 'w') as f:
...
def read_mixed_labels(train_log_dir, filename=LABELS_FILENAME):
    labels_filename = os.path.join(train_log_dir, filename)
    with tf.gfile.Open(labels_filename, 'rb') as f:
        lines = f.read().decode()
    lines = lines.split('\n')
    ...
    for line in lines:
        ...
        labels.update({'labels' + str(label_list_num) : label_list})
        label_list_num = label_list_num + 1
    return labels
