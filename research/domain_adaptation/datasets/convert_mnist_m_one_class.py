...
def _convert_dataset(split_name, filenames, filename_to_class_id, dataset_dir, class_to_convert):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session('') as sess:
            output_filename = _get_output_filename(dataset_dir, split_name, class_to_convert)

            with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
                counter_pic = 1
                counter_num = 1
                for filename in filenames:
                    print(counter_num, '   /  ', counter_pic)
                    class_id = filename_to_class_id[filename]
                    if (class_id) == class_to_convert:
                        # Read the filename:
...
def _get_output_filename(dataset_dir, split_name, class_id):
    ...
    """
    return '%s/mnist_m_%s_%s.tfrecord' % (dataset_dir, split_name, str(class_id))
...
def run(dataset_dir, class_to_convert):
    ...
    Args:
      ...
      class_to_convert: The class to be converted
    """
